...
public partial class VehicleExpensePage
{
	...
	private async Task<bool> LoadExistingTransaction()
	{
		...
		_vehicleExpense = await CommonData.LoadTableDataById<VehicleExpenseModel>(VehicleExpenseNames.VehicleExpense, Id.Value);
	...
	{
		if (!await DataStorageService.LocalExists(StorageFileNames.VehicleExpenseCartDataFileName))
			return false;
	...
	private async Task ResolveExpensesCart()
	{
		try
		{
			...
			if (await DataStorageService.LocalExists(StorageFileNames.VehicleExpenseCartDataFileName))
				_expensesCart = System.Text.Json.JsonSerializer.Deserialize<List<VehicleExpenseDetailsCartModel>>(await DataStorageService.LocalGetAsync(StorageFileNames.VehicleExpenseCartDataFileName));
		}
		catch (Exception ex)
		{
	...
	private async Task<bool> LoadExistingExpensesCart()
	{
		...
		var existingCart = await CommonData.LoadTableDataByMasterId<VehicleExpenseDetailsModel>(VehicleExpenseNames.VehicleExpenseDetails, _vehicleExpense.Id);
	...
	private async Task SaveTransactionFile()
	{
		...
		try
		{
			...
			await DataStorageService.LocalSaveAsync(StorageFileNames.VehicleExpenseCartDataFileName, System.Text.Json.JsonSerializer.Serialize(_expensesCart));
		}
		catch (Exception ex)
		{
	...
	private async Task DeleteLocalFiles()
	{
		...
		await DataStorageService.LocalRemove(StorageFileNames.VehicleExpenseCartDataFileName);
	}
