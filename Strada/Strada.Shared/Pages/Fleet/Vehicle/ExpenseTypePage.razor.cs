...
public partial class ExpenseTypePage
{
	...
	private async Task DeleteRecoverTransaction(int id, bool isRecover)
	{
		try
		{
			...
			var expenseType = await CommonData.LoadTableDataById<ExpenseTypeModel>(FleetNames.ExpenseType, id)
				?? throw new Exception("Transaction not found.");

			if (isRecover) await ExpenseTypeData.RecoverTransaction(expenseType, _user.Id, FormFactor.GetFormFactor() + FormFactor.GetPlatform());
			else await ExpenseTypeData.DeleteTransaction(expenseType, _user.Id, FormFactor.GetFormFactor() + FormFactor.GetPlatform());

			await _toastNotification.ShowAsync("Success", $"Transaction {expenseType.Name} has been {(isRecover ? "recovered" : "deleted")} successfully.", ToastType.Success);
			ResetPage();
		}
		catch (Exception ex)
		{
	...
	private async Task ExportMaster(bool isExcel = false)
	{
		...
		try
		{
			...
			var (stream, fileName) = await ExpenseTypeExport.ExportMaster(_expenseTypes, isExcel ? ReportExportType.Excel : ReportExportType.PDF);
