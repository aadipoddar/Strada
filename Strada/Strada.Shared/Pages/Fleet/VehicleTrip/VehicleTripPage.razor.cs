...
public partial class VehicleTripPage
{
	...
	private async Task ResolvePaymentsCart()
	{
		try
		{
			...
			if (await DataStorageService.LocalExists(StorageFileNames.VehicleTripPaymentsCartDataFileName))
				_paymentsCart = System.Text.Json.JsonSerializer.Deserialize<List<VehicleTripOMCCardPaymentsCartModel>>(await DataStorageService.LocalGetAsync(StorageFileNames.VehicleTripPaymentsCartDataFileName));
		}
		catch (Exception ex)
		{
	...
	private async Task<bool> LoadExistingPaymentsCart()
	{
		...
		var existingCart = await CommonData.LoadTableDataByMasterId<VehicleTripOMCCardPaymentsModel>(FleetNames.VehicleTripOMCCardPayments, _vehicleTrip.Id);
