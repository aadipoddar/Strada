...
public partial class RoutePage
{
	...
	private async Task DeleteRecoverSelectedItem()
	{
		...
		var routeName = $"{_locations.FirstOrDefault(l => l.Id == record.FromLocationId)?.Name} to {_locations.FirstOrDefault(l => l.Id == record.ToLocationId)?.Name}";

		if (record.Status)
			await ShowConfirmation("Delete", $"Are you sure you want to delete {routeName}", () => DeleteTransaction(record.Id));
		else
			await ShowConfirmation("Recover", $"Are you sure you want to recover {routeName}", () => RecoverTransaction(record.Id));
	}
