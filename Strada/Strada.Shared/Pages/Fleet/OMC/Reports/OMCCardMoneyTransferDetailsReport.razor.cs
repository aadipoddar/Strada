...
namespace Strada.Shared.Pages.Fleet.OMC.Reports;

public partial class OMCCardMoneyTransferDetailsReport : IAsyncDisposable
{
	...
	private List<OMCCardMoneyTransferDetailsOverviewModel> _transactionOverviews = [];
	private List<OMCCardMoneyTransferDetailsOverviewModel> _allTransactionOverviews = [];
	...
	private SfGrid<OMCCardMoneyTransferDetailsOverviewModel> _sfGrid;
	...
	private async Task LoadTransactionOverviews()
	{
		...
		try
		{
			...
			_allTransactionOverviews = await CommonData.LoadTableDataByDate<OMCCardMoneyTransferDetailsOverviewModel>(
				FleetNames.OMCCardMoneyTransferDetailsOverview,
	...
	private async Task ApplyFilters()
	{
		_transactionOverviews = [.. _allTransactionOverviews.Where(t =>
				...
				(_selectedOMCCard == null || _selectedOMCCard.Id == 0 || t.OMCCardId == _selectedOMCCard.Id) &&
	...
	private async Task OnOMCCardChanged(OMCCardModel value)
	{
	...
	private async Task ViewSelectedTransaction()
	{
		...
		var decodedTransactionNo = await DecodeCode.DecodeTransactionNo(_sfGrid.SelectedRecords.First().TransactionNo, false, false, CodeType.OMCCardMoneyTransfer);
	...
	private async Task OpenFinancialAccountingPosting()
	{
		...
		{
			await _toastNotification.ShowAsync("No Posting", "This transfer has no linked accounting posting.", ToastType.Warning);
			return;
		}
	...
	private async Task DeleteRecoverTransaction(int id, string transactionNo, bool isRecover)
	{
		...
		try
		{
			...
			var transfer = await CommonData.LoadTableDataById<OMCCardMoneyTransferModel>(FleetNames.OMCCardMoneyTransfer, id)
				?? throw new Exception("Transaction not found.");
			transfer.Status = isRecover;
			transfer.LastModifiedBy = _user.Id;
			transfer.LastModifiedAt = await CommonData.LoadCurrentDateTime();
			transfer.LastModifiedFromPlatform = FormFactor.GetFormFactor() + FormFactor.GetPlatform();

			if (isRecover) await OMCCardMoneyTransferData.RecoverTransaction(transfer);
			else await OMCCardMoneyTransferData.DeleteTransaction(transfer);
	...
	private async Task ExportReport(bool isExcel = false)
	{
		...
		try
		{
			...
			var (stream, fileName) = await OMCCardMoneyTransferReportExport.ExportTransfersReport(
				...
				_showAllColumns,
				_showDeleted,
				_selectedOMCCard?.Id > 0 ? _selectedOMCCard : null,
	...
	private async Task ExportSelectedTransaction(bool isExcel = false)
	{
		...
		try
		{
			...
			var decodeTransactionNo = await DecodeCode.DecodeTransactionNo(_sfGrid.SelectedRecords.First().TransactionNo, !isExcel, isExcel, CodeType.OMCCardMoneyTransfer);
	...
	#region Utilities
	private async Task OnGridContextMenuItemClicked(ContextMenuClickEventArgs<OMCCardMoneyTransferDetailsOverviewModel> args)
	{
		switch (args.Item.Id)
		{
