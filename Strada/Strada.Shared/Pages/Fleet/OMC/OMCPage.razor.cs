...
public partial class OMCPage
{
	...
	private async Task DeleteRecoverTransaction(int id, bool isRecover)
	{
		try
		{
			...
			var omc = await CommonData.LoadTableDataById<OMCModel>(FleetNames.OMC, id)
				?? throw new Exception("Transaction not found.");

			if (isRecover) await OMCData.RecoverTransaction(omc, _user.Id, FormFactor.GetFormFactor() + FormFactor.GetPlatform());
			else await OMCData.DeleteTransaction(omc, _user.Id, FormFactor.GetFormFactor() + FormFactor.GetPlatform());

			await _toastNotification.ShowAsync("Success", $"Transaction {omc.Name} has been {(isRecover ? "recovered" : "deleted")} successfully.", ToastType.Success);
			ResetPage();
		}
		catch (Exception ex)
		{
	...
	private async Task ExportMaster(bool isExcel = false)
	{
		...
		try
		{
			...
			var (stream, fileName) = await OMCExport.ExportMaster(_omcs, isExcel ? ReportExportType.Excel : ReportExportType.PDF);
