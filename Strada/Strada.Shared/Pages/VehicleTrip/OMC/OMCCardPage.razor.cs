...
using StradaLibrary.Data.VehicleTrip.OMC;
...
namespace Strada.Shared.Pages.VehicleTrip.OMC;
...
public partial class OMCCardPage
{
	...
	{
		_omcCards = await CommonData.LoadTableData<OMCCardModel>(VehicleTripNames.OMCCard);
		_omcs = await CommonData.LoadTableData<OMCModel>(VehicleTripNames.OMC);
	...
	private async Task ConfirmDelete()
	{
		try
		{
			...
			var omcCard = await CommonData.LoadTableDataById<OMCCardModel>(VehicleTripNames.OMCCard, _deleteTransactionId)
	...
	private async Task ConfirmRecover()
	{
		try
		{
			...
			var omcCard = await CommonData.LoadTableDataById<OMCCardModel>(VehicleTripNames.OMCCard, _recoverTransactionId)
				?? throw new Exception("Transaction not found.");
	...
	private async Task EditSelectedItem()
	{
		...
		_omcCard = await CommonData.LoadTableDataById<OMCCardModel>(VehicleTripNames.OMCCard, selectedRecords[0].Id);
		if (_omcCard is null)
