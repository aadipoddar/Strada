...
namespace Strada.Shared.Pages.VehicleTrip.TripAdvance.Reports;

public partial class TripAdvanceReport : IAsyncDisposable
{
	...
	private List<TripAdvanceOverviewModel> _transactionOverviews = [];
	...
	private SfGrid<TripAdvanceOverviewModel> _sfGrid;
	...
	private async Task LoadTransactionOverviews()
	{
		...
		try
		{
			...
			_transactionOverviews = await CommonData.LoadTableDataByDate<TripAdvanceOverviewModel>(
				VehicleTripNames.TripAdvanceOverview,
	...
	private async Task ExportExcel()
	{
		...
		try
		{
			...
			var (stream, fileName) = await TripAdvanceReportExport.ExportReport(
	...
	private async Task ConfirmDelete()
	{
		...
		try
		{
			...
			var trip = await CommonData.LoadTableDataById<TripAdvanceModel>(VehicleTripNames.TripAdvanceOverview, _deleteTransactionId)
			...
			await TripAdvanceData.DeleteTransaction(trip);
	...
	private async Task ConfirmRecover()
	{
		...
		try
		{
			...
			var trip = await CommonData.LoadTableDataById<TripAdvanceModel>(VehicleTripNames.TripAdvanceOverview, _recoverTransactionId)
				?? throw new Exception("Transaction not found.");
			trip.Status = true;
			trip.LastModifiedBy = _user.Id;
			trip.LastModifiedAt = await CommonData.LoadCurrentDateTime();
			trip.LastModifiedFromPlatform = FormFactor.GetFormFactor() + FormFactor.GetPlatform();
			await TripAdvanceData.RecoverTransaction(trip);
	...
	private async Task OnMenuSelected(Syncfusion.Blazor.Navigations.MenuEventArgs<Syncfusion.Blazor.Navigations.MenuItem> args)
	{
		...
		{
			case "NewTransaction":
				await AuthenticationService.NavigateToRoute(PageRouteNames.TripAdvance, FormFactor, JSRuntime, NavigationManager);
				break;
			case "Refresh":
				await LoadTransactionOverviews();
				break;
	...
	private async Task OnGridContextMenuItemClicked(ContextMenuClickEventArgs<TripAdvanceOverviewModel> args)
	{
		switch (args.Item.Id)
		{
			case "View":
