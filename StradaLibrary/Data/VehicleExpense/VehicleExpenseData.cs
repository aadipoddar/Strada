...
namespace StradaLibrary.Data.VehicleExpense;
...
public static class VehicleExpenseData
{
	private static async Task<int> InsertVehicleExpense(VehicleExpenseModel vehicleExpense, SqlDataAccessTransaction sqlDataAccessTransaction = null) =>
		(await SqlDataAccess.LoadData<int, dynamic>(VehicleExpenseNames.InsertVehicleExpense, vehicleExpense, sqlDataAccessTransaction)).FirstOrDefault();
	...
	private static async Task<int> InsertVehicleExpenseDetails(VehicleExpenseDetailsModel vehicleExpenseDetails, SqlDataAccessTransaction sqlDataAccessTransaction = null) =>
		(await SqlDataAccess.LoadData<int, dynamic>(VehicleExpenseNames.InsertVehicleExpenseDetails, vehicleExpenseDetails, sqlDataAccessTransaction)).FirstOrDefault();
	...
	{
		expense.Status = true;
		var expenseDetails = await CommonData.LoadTableDataByMasterId<VehicleExpenseDetailsModel>(VehicleExpenseNames.VehicleExpenseDetails, expense.Id);
		
		await SaveTransaction(expense, expenseDetails);
	...
	private static async Task<VehicleExpenseModel> ValidateTransaction(VehicleExpenseModel expense, bool update, SqlDataAccessTransaction sqlDataAccessTransaction)
	{
		...
		if (update)
		{
			var existingExpense = await CommonData.LoadTableDataById<VehicleExpenseModel>(VehicleExpenseNames.VehicleExpense, expense.Id, sqlDataAccessTransaction)
	...
	private static async Task SaveExpensesDetail(VehicleExpenseModel expense, List<VehicleExpenseDetailsModel> expensesDetails, bool update, SqlDataAccessTransaction sqlDataAccessTransaction)
	{
		if (update)
		{
			var existingExpensesDetails = await CommonData.LoadTableDataByMasterId<VehicleExpenseDetailsModel>(VehicleExpenseNames.VehicleExpenseDetails, expense.Id, sqlDataAccessTransaction);
		...
		foreach (var item in expensesDetails)
		{
			...
			if (id <= 0)
				throw new InvalidOperationException("Failed to save vehicle expense details item.");
		}
	}
}