...
public static class VehicleRouteLocationData
{
	public static async Task<int> InsertRouteLocation(VehicleRouteLocationModel routeLocation) =>
		(await SqlDataAccess.LoadData<int, dynamic>(VehicleTripNames.InsertVehicleRouteLocation, routeLocation)).FirstOrDefault();
	...
	private static async Task ValidateTransaction(VehicleRouteLocationModel routeLocation)
	{
		...
		var allLocations = await CommonData.LoadTableData<VehicleRouteLocationModel>(VehicleTripNames.VehicleRouteLocation);
