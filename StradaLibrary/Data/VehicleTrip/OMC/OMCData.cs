...
public static class OMCData
{
	public static async Task<int> InsertOMC(OMCModel omc) =>
		(await SqlDataAccess.LoadData<int, dynamic>(VehicleTripNames.InsertOMC, omc)).FirstOrDefault();
	...
	private static async Task ValidateTransaction(OMCModel omc)
	{
		...
		var allOMCs = await CommonData.LoadTableData<OMCModel>(VehicleTripNames.OMC);
