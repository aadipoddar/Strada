...
internal static class VehicleTripBillNotify
{
    ...
    {
        var trip = await CommonData.LoadTableDataById<VehicleTripBillOverviewModel>(FleetNames.VehicleTripBillOverview, tripId);

        var emailData = new TransactionMailing.TransactionEmailData
        {
            TransactionType = "Vehicle Trip BILL",
        ...
        {
            var (afterStream, afterFileName) = await VehicleTripInvoiceExport.ExportInvoice(tripId, InvoiceExportType.PDF);
        ...
        else
        {
            ...
            var (pdfStream, pdfFileName) = await VehicleTripInvoiceExport.ExportInvoice(tripId, InvoiceExportType.PDF);
