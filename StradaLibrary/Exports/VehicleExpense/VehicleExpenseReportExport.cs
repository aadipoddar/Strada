...
public static class VehicleExpenseReportExport
{
	public static async Task<(MemoryStream stream, string fileName)> ExportReport(
		IEnumerable<VehicleExpenseOverviewModel> expensesData,
	...
	public static async Task<(MemoryStream stream, string fileName)> ExportDetailsReport(
		IEnumerable<VehicleExpenseDetailsOverviewModel> detailsData,
		ReportExportType exportType,
		DateOnly? dateRangeStart = null,
		DateOnly? dateRangeEnd = null,
		bool showAllColumns = true,
		...
		VehicleModel vehicle = null)
	{
		...
		string fileName = $"VEHICLE_EXPENSES_DETAILS_REPORT";
		if (dateRangeStart.HasValue || dateRangeEnd.HasValue)
			fileName += $"_{dateRangeStart?.ToString("yyyyMMdd") ?? "START"}_to_{dateRangeEnd?.ToString("yyyyMMdd") ?? "END"}";
		...
		if (exportType == ReportExportType.PDF)
		{
			var stream = await PDFReportExportUtil.ExportToPdf(
				detailsData,
				"VEHICLE EXPENSES DETAILS REPORT",
				dateRangeStart,
				dateRangeEnd,
				columnSettings,
				columnOrder,
